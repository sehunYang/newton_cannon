...
y 범위는 **그 비행의 데이터에 맞춰** 잡습니다. 그래서 눈금이 발사의 에너지 규모를 따라갑니다
(2 km/s → 2 MJ/kg, 10 km/s → 69 MJ/kg).
...
대신 "E < 0 이면 속박" 이 "E < 62.5 MJ/kg 이면 속박" 이 되므로, 그 선이 화면에 들어올 만한
발사(≥ 0.6배)에서는 `탈출` 점선을 함께 그립니다.
