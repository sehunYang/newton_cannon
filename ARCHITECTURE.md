...
축은 x·y 모두 축선을 긋고 눈금을 답니다(`niceStep` 은 지표면 거리 눈금과 공용이라
`core/format.js` 에 있습니다). 좁은 패널이라 두 가지를 아낍니다 — 시간 축은 전체를 한
단위(s / min / hr)로 통일해 눈금에는 숫자만 적고 단위는 오른쪽 끝에 한 번만 붙이며,
세로 단위(MJ/kg)는 패널 제목이 이미 말하므로 생략합니다. 왼쪽 여백은 상수가 아니라
**가장 넓은 눈금 글자를 재서** 정합니다 — 축을 끊으면 `24.50` 처럼 자릿수가 늘어나
`40` 짜리 여백으로는 글자가 잘립니다. 축을 끊은 경우에는 두 밴드에 각각 눈금을 답니다
(빈 구간에는 눈금을 두지 않습니다 — 거기엔 읽을 값이 없습니다).
