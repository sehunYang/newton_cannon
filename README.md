...
싶으면 `F` 키(또는 **포탄 추적** 체크 해제)로 추적을 끄세요 — 지구를 가운데 두고 **타원 전체가
보이도록** 물러납니다. 착탄·탈출로 비행이 끝나면 자동으로 그렇게 됩니다.

`L` 키(또는 **실제 축척** 체크 해제)로 **압축 보기**로 바꾸면 먼 곳을 로그 눈금으로 눌러
탈출 궤도까지 한 화면에 보이지만, 그 대신 **모양이 왜곡**됩니다 — 반지름을 로그로 누르기 때문에
멀리 나가는 타원일수록 원지점 쪽이 뾰족한 **달걀꼴**이 되고 초점도 지구에서 벗어납니다.
화면 좌상단에 `축척 압축 · 모양 왜곡` 배지가 떠 있는 동안은 모양을 믿지 마세요.
...
| **궤적 표시** / `T` | 지나온 길 표시 |
| **격자 표시** / `G` | 거리·고도 눈금 표시 |
| **실제 축척** / `L` | 켜면 실제 비율(타원이 정확한 타원), 끄면 압축 보기(먼 곳을 로그로 압축, 모양 왜곡) |
| **포탄 추적** / `F` | 켜면 카메라가 포탄을 따라감, 끄면 지구 중앙 + 궤도 전체 보기 (실제 축척에서만) |
