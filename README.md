...
화면은 언제나 **실제 축척**입니다 — 화면의 타원은 수학 시간에 배운 그 타원이고,
**지구 중심이 초점**에 있습니다.
...
가장자리에 둥근 창**이 떠서 미니 지구와 지금 거리를 보여 줍니다. 궤도 전체를 한눈에 보고
싶으면 `F` 키(또는 **포탄 추적** 체크 해제)로 추적을 끄세요 — 카메라가 뒤로 물러나 **타원 전체**를
한 화면에 담습니다(위 왼쪽 그림). 착탄·탈출로 비행이 끝나면 자동으로 그렇게 됩니다.

멀리 있는 궤도를 보는 방법은 이것 하나뿐입니다 — **카메라를 물리는 것**. 예전에는 먼 거리를
로그 눈금으로 눌러 담는 '압축 보기'도 있었지만, 그러면 타원이 원지점 쪽만 뾰족한 **달걀꼴**로
일그러지고 초점도 지구에서 벗어나 "궤도는 타원이다"를 가르칠 수 없어 없앴습니다.
화면에 보이는 모양은 언제나 실제 모양입니다.
...
| **궤적 표시** / `T` | 지나온 길 표시 |
| **격자 표시** / `G` | 거리·고도 눈금 표시 |
| **포탄 추적** / `F` | 켜면 카메라가 포탄을 따라감, 끄면 물러나서 궤도 전체 보기 |
| **에너지 그래프** / `E` | 운동·퍼텐셜·역학적 에너지 실시간 그래프 |
...
### 에너지 그래프 (데스크톱은 좌측 · 모바일은 HUD 아래)

![에너지 그래프](docs/images/02c-energy-graph.png)

발사하면 **1 kg 당** 세 에너지가 시간에 따라 그려집니다.

| 선 | 뜻 | 식 |
|---|---|---|
| 🟠 **운동** | 빠를수록 큼 | K = v²/2 |
| 🔵 **퍼텐셜** | 높이 올라갈수록 큼 — **발사 지점이 0** | U = GM/R발사 − GM/r |
| 🟢 **역학적** | 둘의 합 — **수평선** | E = K + U |

질량을 가정하지 않은 이유는 궤도가 질량과 무관하기 때문입니다. m 을 곱하면 세 값이 모두
같은 배율로 커질 뿐 그래프 모양은 똑같습니다.

이 그래프가 한 화면에서 보여주는 것:

- **K 와 U 가 정확히 반대로 출렁이는데 그 합은 평평합니다.** 근지점에서 빠르고 원지점에서
  느린 이유가 바로 이 교환입니다 — "어디서 온 에너지인가"에 대한 답이 눈에 보입니다.
- **눈금은 그 발사의 에너지 규모에 자동으로 맞춰집니다.** 2 km/s 발사면 축이 2 MJ/kg,
  9 km/s 면 43 MJ/kg 로 잡혀서, 어떤 발사에서도 곡선이 패널을 가득 채웁니다.
  역학적 에너지가 보존되니 **쏘는 순간 이미 결정된 값**입니다 — 궤도가 정해지면 근지점·원지점이
  정해지고, 거기서 K 와 U 가 어디까지 갈지가 정해지니까요. 그래서 축은 대개 정확히
  **0 부터 그 발사의 역학적 에너지까지**이고, 비행 도중 눈금이 바뀌지 않습니다.
- **변화가 너무 작으면 축을 끊어서 확대합니다.** 등속 원운동보다 느리게 **수평으로** 쏘면
  발사 지점이 곧 궤도의 가장 높은 지점이라, 포탄이 오르내리는 높이가 에베레스트 높이뿐입니다.
  7 km/s 수평 발사에서 교환되는 에너지는 0.087 MJ/kg — 24.5 MJ/kg 짜리 축에서는 0.3 픽셀,
  즉 안 보입니다. 그런데 그 축의 99% 는 **아무 곡선도 지나지 않는 빈 구간**이라, 거기를
  물결선(`〰 24.5 생략`)으로 끊고 두 띠만 약 100배 확대해서 그립니다. 잘라낸 곳에는 데이터가
  없고 띠 안의 배율은 일정하므로 곡선의 모양은 그대로입니다.
- **초록 선이 `탈출` 점선을 넘으면 돌아오지 않습니다.** 탈출선은 62.5 MJ/kg — 지구 중력을
  이기는 데 필요한 에너지(= ½v₂², v₂ = 제2 우주속도)입니다. 9.8 km/s 이상으로 쏘면
  이 선이 화면에 함께 나타납니다(그보다 느린 발사에서는 선을 넣느라 곡선을 눌러 버리게 되어
  그리지 않습니다).
- 수치 적분(속도 베를레)이 에너지를 정말 보존하는지도 함께 보입니다. 1회 공전 동안
  E 의 변동은 **0.0003% 수준**이라 육안으로는 완전한 수평선입니다.

> 📐 **퍼텐셜의 기준점에 대하여.** 대학 교재와 이 시뮬레이터의 궤도 판정은 무한히 먼 곳을
> 0 으로 잡아 U = −GM/r < 0 을 쓰고, "E < 0 이면 속박"이라고 말합니다. 그래프만 **발사 지점을
> 0 으로** 옮긴 이유는 눈금 때문입니다 — 무한대 기준으로 그리면 축의 대부분을 정보가 없는
> 상수(−62.5 MJ/kg)가 차지해서 2 km/s 발사의 변화가 축의 **1.6%(약 2 px)** 밖에 안 됩니다.
> 상수를 더한 것뿐이라 곡선의 모양·보존·교환 관계는 완전히 같고, 지표 근처에서는 U ≈ gh 라
> 교과서의 mgh 와 곧바로 이어집니다(고도 100 km 에서 0.96 MJ/kg ≈ 9.8 × 100 km).
> 두 기준의 "탈출" 조건도 같은 말입니다 — E<sub>무한대</sub> = 0 ⟺ E<sub>발사지점</sub> = 62.5 MJ/kg.

### 궤적의 색 = 그 순간의 속력

<img src="docs/images/03-escape.png" alt="탈출 궤도" width="600">
<br><sub>탈출 궤도 — 지구와 포탄이 함께 들어오도록 카메라가 물러난 결과 화면 (30 Re 를 넘는 순간 탈출로 끝남)</sub>
...
타원 궤도(예: 9 km/s, 0°)를 쏘고 **왼쪽 에너지 그래프**를 함께 관찰합니다.

> **발문** — "높이 올라가면 느려지고 내려오면 빨라진다. 어디서 온 에너지일까?"

최고고도 지점과 근지점의 속력을 기록해 $\frac12 v^2 - \frac{GM}{r}$ 이 일정한지 확인시킵니다.

> **이어지는 발문** — "역학적 에너지 선을 `탈출` 점선 위로 올리려면 초기 속도를 얼마로 해야 할까?"
> (11.18 km/s 를 넘기면 초록 선이 점선 위로 올라가고, 포탄은 돌아오지 않습니다)
...
| 궤도 시점에서 산이 안 보임 | 에베레스트 8,848 m 는 지구 반지름의 0.14%라 실제 비율로는 픽셀 이하입니다. 과장하지 않았고, 그 대신 **지표면 시점**에서 실제 비율로 보여 줍니다 |

> 🔬 **물리 검증.** `npm run verify:physics` 가 17가지 발사각·속도 조합에 대해 시뮬레이터 궤적을
> 해석적 원뿔곡선 `r(θ) = p / (1 + e·cos(θ − ω))` 과 비교합니다. 타원·포물선·쌍곡선 모두
> 반지름 상대오차 **10⁻⁴ 이내**(에너지·각운동량 드리프트 10⁻⁶ 이하)이고, 화면에 그려진 궤적도
> 0.2% 이내로 같은 곡선입니다 — 화면이 월드의 순수한 확대·축소이기 때문입니다.
