...
| **7.9 km/s** | 지면을 **아슬아슬하게 스치고** 추락 | 근지점이 지표면보다 4 km 낮음 — 딱 4 km 모자람! |
| **8.0 km/s** | 떨어지지 않고 **지구를 한 바퀴** (주기 88분) | 제1 우주속도 돌파 |
...
> 🔥 **7.9와 8.0 사이가 이 수업의 하이라이트입니다.**
> 슬라이더는 100 m/s 단위라 7,900에서는 아깝게 추락하고 8,000에서 궤도에 남습니다.
> 겨우 100 m/s 차이가 "떨어진다"와 "영원히 돈다"를 가른다는 걸 학생들이 직접 봅니다.
...
> 💡 **긴 비행은 배속을 올리세요.** 4 km/s로 45° 발사하면 실제 비행시간이 **12분**입니다.
> 지표면 시점은 리얼타임(×1)이 기본이지만, `3`(×32)을 누르면 20초 만에 끝납니다.
...
**궤도 배지**는 지금 **고도에서의** 우주속도로 판정합니다.
높이 올라갈수록 필요한 원궤도 속도가 줄어들기 때문에, 같은 속력이라도 고도에 따라
`포물선 낙하` → `타원/원 궤도` 로 배지가 바뀌는 것을 볼 수 있습니다.
...
발사각 0°로 두고 속도를 100 m/s씩 올리며 처음으로 한 바퀴를 완주하는 값을 찾게 합니다.

- 학생이 찾는 답: **8,000 m/s** (7,900에서는 근지점이 지표면보다 4 km 낮아 추락)
...
> 🔎 **왜 슬라이더로는 딱 맞출 수 없을까?** 슬라이더 단위가 100 m/s이기 때문입니다.
> "실제 경계는 7,904와 7,905 사이 어딘가"라는 점에서 **측정의 분해능** 이야기로 이어갈 수 있습니다.

### 활동 2. 최대 사거리를 만드는 발사각은 정말 45°일까? `심화` ⭐

> **발문** — "교과서에서는 45°가 최대 사거리라고 배웠는데, 여기서도 그럴까?"

같은 속도로 발사각만 바꿔 가며 사거리를 기록하게 합니다. **45°가 아닙니다.**

| 초기속도 | 최대 사거리 발사각 | 그때 사거리 |
|---|---|---|
| 1 km/s | 43° | 112 km |
| 2 km/s | 43° | 431 km |
| 3 km/s | 42° | 1,000 km |
...
빨라질수록 최적각이 **작아집니다.** 왜일까요?
교과서의 45°는 ① 지면이 평평하고 ② 중력이 어디서나 같다는 가정에서 나온 값입니다.
실제로는 지면이 휘어 도망가고, 높이 올라가면 중력이 약해집니다.
**지표면 시점의 `평평한 지구라면` 점선이 바로 그 ①번 가정이 틀렸다는 증거입니다.**
...
| 궤도 시점의 산 높이 8배 과장 | 8,848 m는 지구 반지름의 0.14%라 실제 비율로는 보이지 않습니다. **지표면 시점에서는 실제 비율**로 그립니다 |
...
**Q. 시간이 너무 오래 걸려요.**
시뮬속도 버튼(`1`·`2`·`3`·`4`)으로 최대 128배까지 빨리 감을 수 있습니다.
